...
// Genera schema.org/Product por cada item din�micamente.
// Si la p�gina define window.PM7_CATEGORY antes de cargar
// este script, filtra el listado por esa categor�a.
// --------------------------------------------------------
(function injectProductSchema() {
  try {
    ...
    var listElements = items.map(function (p, i) {
      ...
      return {
        ...
        "item": {
          ...
          "sku": "PM7-" + String(p.id),
          "category": p.category,
          "brand": { "@type": "Brand", "name": "Pasi�n Matera 7" },
          "offers": {
            "@type": "Offer",
            "url": productUrl,
            "priceCurrency": "ARS",
            ...
            "seller": { "@type": "Organization", "name": "Pasi�n Matera 7" }
          }
        }
      };
    });
    ...
    var schema = {
      ...
      "@type": "ItemList",
      "name": category ? ("Cat�logo � " + category) : "Cat�logo Pasi�n Matera 7",
  ...
  } catch (e) { /* silencio: si algo falla, no rompemos la p�gina */ }
})();
